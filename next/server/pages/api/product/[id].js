...
module.exports = __webpack_require__("kh/U");
...
function getProduct(req, res) {
  ...
  const product = _utils_sample_data__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"].find(ex => ex.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === quer);
  ...
  if (typeof product === 'undefined') {
    ...
    const nameList = [];
    Object.values(_utils_sample_data__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"]).map(ex => nameList.push({
      name: ex.name,
      id: ex.id
    }));
    res.json({
